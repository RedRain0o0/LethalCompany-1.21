...
$tellraw @a[tag=lcmc.player.Debug.AI] {"translate":"lcmc.aiDebug.raycast","with":[{"text":"$(entity)"},{"text":"$(id)"},{"text":"$(distance)"},{"text":"$(hit)"},{"text":"$(cansee)"}]}
$execute if score hit lcmc.logic.disposable matches 0 as @n[tag=lcmc.entity.$(entity).ai.$(id).Path.$(cn),distance=...15] run function lcmc:entity/logic/pathfinding/can_see with storage lcmc:disposable
...
execute if score hit lcmc.logic.disposable matches 0 if score distance lcmc.logic.disposable matches ..500 positioned ^ ^ ^0.1 run function lcmc:entity/logic/pathfinding/raycast with storage lcmc:disposable